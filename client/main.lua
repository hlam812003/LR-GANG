...
function LR_GANG:EventHandler()
    ...
    AddEventHandler("lr_gang:client:syncBlip", function(gangHouse)
        ...
        for k, v in pairs(gangHouse) do 
            ...
            AddTextComponentSubstringPlayerName(v.owner or "<FONT FACE='Oswald'><b>".."Nhà Gang Trống".."</b></FONT>")
    ...
    RegisterNUICallback("OpenBossMenu", function(data, cb)
        ...
        TriggerEvent("esx_society:openBossMenu2", self.gangData.gangName, function(data, menu)
            menu.close()
        end, {wash = true})
    end)
end
...
function LR_GANG:Open()
    if self.PlayerData.job2.name == "unemployed2" then 
            ...
            if self.gangData.inventoryPos and self.gangData.inventoryPos.x then 
                ...
                if distance <= 20.0 then 
                    ...
                    if distance <= 1.0 then 
                        ...
                        if IsControlJustReleased(0, 38) then 
                            TriggerEvent("esx_inventoryhud:openSociety2Inventory2")
                        end
                    end
                end
            end
                ...
                if IsControlJustReleased(0, 38) then 
                    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'create_gang', {
                        ...
                        if gangLabel == nil then 
                            ESX.ShowNotification("Tên  không được để trôngs")
                        else

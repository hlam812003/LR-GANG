...
AddEventHandler("lr_gang:server:createGang", function(gangName, gangLabel)
    ...
    if xPlayer.getMoney() >= 1000000 and not Gangs[gangName] then 
        ...
        xPlayer.showNotification("Không đủ ~y~1.000.000$~w~ để tạo băng đảnh")
    end
end)
    ...
    if xPlayer.job2.grade_name == "boss" then
        if Gangs[xPlayer.job2.name] then
            ...
            if house ~= nil then 
                ...
                xPlayer.showNotification("Gang của bạn  nhà nên không thể đặt kho đồ")
            end
        end
    else
        xPlayer.showNotification("Bạn không đủ quyền hạn")
...
AddEventHandler("lr_gang:server:setHouse", function(index, sprite)
    ...
    if xPlayer.job2.grade_name == "boss" then 
        ...
        if Gangs[xPlayer.job2.name].gangLevel >= 3 then
            ...
            xPlayer.showNotification("Gang của bạn chưa đạt cấp 2 để sử dụng chức năng này")
        end
    end
end)
